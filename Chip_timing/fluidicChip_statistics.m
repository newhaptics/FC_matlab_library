classdef fluidicChip_statistics
    ...
    methods
        ...
        function FC = get_cellData(FC,location,input)
            ...
            A = FC.store_brightnessData(avgBright,df,pulseDf,pulseBright,input,A);
    ...
    methods (Access = private)
        ...
        function [avgBright, df, pulseBright, pulseDf] = get_brightData(FC,location,input)
            ...
            if input == 0
                ...
                pulseBright = FC.brightMatrix(row,column).pulseBright0;
                pulseDf = FC.brightMatrix(row,column).pulseDf0;            
            end
            if input == 1
                avgBright = FC.brightMatrix(row,column).avgBright1;
                df = FC.brightMatrix(row,column).df1;
                pulseBright = FC.brightMatrix(row,column).pulseBright1;
                pulseDf = FC.brightMatrix(row,column).pulseDf1;      
            end
            
            
        end
        
        function brightData = store_brightnessData(~,avgBright,df,pulseDf,pulseBright,input,Data)
            
            % if input is 0
            if input == 0
                Data.avgBright0 = avgBright;
                Data.df0 = df;
                Data.pulseBright0 = pulseBright;
                Data.pulseDf0 = pulseDf;
            end
            ...
            if input == 1
                ...
                Data.df1 = df;
                Data.pulseBright1 = pulseBright;
                Data.pulseDf1 = pulseDf;
            end
